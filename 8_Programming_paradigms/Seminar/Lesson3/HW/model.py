...
class Model:
    ...
    def check_if_win(self) -> bool:
        ...
        for i in range(0, table_size):
            if self.table.cells[i][0] != -1 and (
                    self.table.cells[i][0] == self.table.cells[i][1] == self.table.cells[i][2]):
                return True
            if self.table.cells[0][i] != -1 and (
                    self.table.cells[0][i] == self.table.cells[1][i] == self.table.cells[2][i]):
                return True
        # Проверяем диагонали
